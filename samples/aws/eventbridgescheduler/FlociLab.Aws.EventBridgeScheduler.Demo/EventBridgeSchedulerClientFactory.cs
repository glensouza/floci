...
/// <c>Amazon*Config</c> shares; Scheduler needs none of its own — every operation already
/// addresses a single base endpoint and carries its schedule name in the request path. Nothing
/// else about the SDK usage differs from production.
/// </summary>
public sealed class EventBridgeSchedulerClientFactory(AwsEndpoints endpoints)
{
    ...
    /// <summary>For building the fake target ARN and role the demo run never dereferences.</summary>
    ...
    /// </summary>
    public IAmazonScheduler Create()
    {
        ...
        {
            return new AmazonSchedulerClient(new AmazonSchedulerConfig
            {
                RegionEndpoint = Amazon.RegionEndpoint.GetBySystemName(endpoints.Region),
            });
        }

        AmazonSchedulerConfig config = new AmazonSchedulerConfig
        {
            ...
            // A production app against real Scheduler wants the retries; this is the second and
            // last emulator-shaped line in the sample.
            MaxErrorRetry = 0,
        }.ForFloci(endpoints);

        return new AmazonSchedulerClient(endpoints.Credentials(), config);
    }
}
