...
public static class ServiceCollectionExtensions
{
    ...
    public static IServiceCollection AddAwsIamDemo(this IServiceCollection services)
    {
        ...
        services.TryAddSingleton<IamClientFactory>();

        // Registered by concrete type as well as by interface, because IamPage injects
        // IamDemo directly — a page that owns one service has no use for the whole catalog,
        // and the interface registration below forwards to the same instance rather than
        // building a second one.
        services.TryAddSingleton<IamDemo>();
        ...
        // register IAM twice if a host called this method twice. TryAddEnumerable de-duplicates
        // on the implementation type, which is both.
        services.TryAddEnumerable(
            ServiceDescriptor.Singleton<IServiceDemo, IamDemo>(sp => sp.GetRequiredService<IamDemo>()));
