...
/// <c>Amazon*Config</c> shares; KMS needs none of its own, like SQS and DynamoDB — every operation
/// already addresses a single base endpoint and carries its key id in the request body. Nothing
/// else about the SDK usage differs from production.
/// </summary>
public sealed class KmsClientFactory(AwsEndpoints endpoints)
{
    ...
    /// </summary>
    public IAmazonKeyManagementService Create()
    {
        ...
        AmazonKeyManagementServiceConfig config = new AmazonKeyManagementServiceConfig
        {
            ...
            // A production app against real KMS wants the retries; this is the second and last
            // emulator-shaped line in the sample.
            MaxErrorRetry = 0,
        }.ForFloci(endpoints);

        return new AmazonKeyManagementServiceClient(endpoints.Credentials(), config);
    }
}
