...
/// <c>Amazon*Config</c> shares; Secrets Manager needs none of its own, like SQS, DynamoDB and KMS —
/// every operation already addresses a single base endpoint and carries its secret id in the
/// request body. Nothing else about the SDK usage differs from production.
/// </summary>
public sealed class SecretsManagerClientFactory(AwsEndpoints endpoints)
{
    ...
    /// </summary>
    public IAmazonSecretsManager Create()
    {
        ...
        AmazonSecretsManagerConfig config = new AmazonSecretsManagerConfig
        {
            ...
            // A production app against real Secrets Manager wants the retries; this is the second
            // and last emulator-shaped line in the sample.
            MaxErrorRetry = 0,
        }.ForFloci(endpoints);

        return new AmazonSecretsManagerClient(endpoints.Credentials(), config);
    }
}
