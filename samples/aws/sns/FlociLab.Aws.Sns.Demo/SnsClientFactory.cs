...
/// <c>Amazon*Config</c> shares; SNS needs none of its own, like SQS — every operation already
/// addresses a single base endpoint and carries its topic ARN in the request body. Nothing else
/// about the SDK usage differs from production.
/// </summary>
public sealed class SnsClientFactory(AwsEndpoints endpoints)
{
    ...
    /// </summary>
    public IAmazonSimpleNotificationService Create()
    {
        ...
        AmazonSimpleNotificationServiceConfig config = new AmazonSimpleNotificationServiceConfig
        {
            ...
            // A production app against real SNS wants the retries; this is the second and last
            // emulator-shaped line in the sample.
            MaxErrorRetry = 0,
        }.ForFloci(endpoints);

        return new AmazonSimpleNotificationServiceClient(endpoints.Credentials(), config);
    }
}
