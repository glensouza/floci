...
/// <c>Amazon*Config</c> shares; SQS needs none of its own, unlike S3's <c>ForcePathStyle</c> —
/// every operation already addresses a single base endpoint and carries its queue in the request
/// body. <c>MaxErrorRetry</c> is a lab-ergonomics choice rather than a compatibility one, explained
/// at its assignment. Nothing else about the SDK usage differs from production.
/// </summary>
public sealed class SqsClientFactory(AwsEndpoints endpoints)
{
    ...
    public IAmazonSQS Create()
    {
        ...
        AmazonSQSConfig config = new AmazonSQSConfig
        {
            ...
            // A production app against real SQS wants the retries; this is the second and last
            // emulator-shaped line in the sample.
            MaxErrorRetry = 0,
        }.ForFloci(endpoints);

        return new AmazonSQSClient(endpoints.Credentials(), config);
    }
}
