...
/// <c>Amazon*Config</c> shares; SSM needs none of its own, like SecretsManager, SQS and DynamoDB —
/// every operation already addresses a single base endpoint and carries its parameter name in the
/// request body. Nothing else about the SDK usage differs from production.
/// </summary>
public sealed class SsmClientFactory(AwsEndpoints endpoints)
{
    ...
    /// </summary>
    public IAmazonSimpleSystemsManagement Create()
    {
        ...
        {
            return new AmazonSimpleSystemsManagementClient(new AmazonSimpleSystemsManagementConfig
            {
        ...
        AmazonSimpleSystemsManagementConfig config = new AmazonSimpleSystemsManagementConfig
        {
            ...
            // A production app against real SSM wants the retries; this is the second and last
            // emulator-shaped line in the sample.
            MaxErrorRetry = 0,
        }.ForFloci(endpoints);

        return new AmazonSimpleSystemsManagementClient(endpoints.Credentials(), config);
    }
}
