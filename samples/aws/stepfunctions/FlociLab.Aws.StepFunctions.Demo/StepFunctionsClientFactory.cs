...
/// <c>Amazon*Config</c> shares; Step Functions needs none of its own — every operation already
/// addresses a single base endpoint and carries its state machine or execution ARN in the request
/// body. Nothing else about the SDK usage differs from production.
/// </summary>
public sealed class StepFunctionsClientFactory(AwsEndpoints endpoints)
{
    ...
    /// </summary>
    public IAmazonStepFunctions Create()
    {
        ...
        {
            return new AmazonStepFunctionsClient(new AmazonStepFunctionsConfig
            {
        ...
        AmazonStepFunctionsConfig config = new AmazonStepFunctionsConfig
        {
            ...
            // A production app against real Step Functions wants the retries; this is the second
            // and last emulator-shaped line in the sample.
            MaxErrorRetry = 0,
        }.ForFloci(endpoints);

        return new AmazonStepFunctionsClient(endpoints.Credentials(), config);
    }
}
