...
public static class ServiceCollectionExtensions
{
    ...
    /// this line, and nothing else. There is no capability registration — SWF's plan row names
    /// none (docs/BLAZOR-PLAN.md §13), so it appears only in its own provider's nav, not on a
    /// comparison page.
    /// </summary>
    public static IServiceCollection AddAwsSwfDemo(this IServiceCollection services)
    {
        ArgumentNullException.ThrowIfNull(services);

        services.TryAddSingleton<SwfClientFactory>();

        // Registered by concrete type as well as by interface, because SwfPage injects SwfDemo
        ...
        // second one.
        services.TryAddSingleton<SwfDemo>();
        ...
        // register SWF twice if a host called this method twice. TryAddEnumerable de-duplicates
        // on the implementation type, which is both.
        services.TryAddEnumerable(
            ServiceDescriptor.Singleton<IServiceDemo, SwfDemo>(sp => sp.GetRequiredService<SwfDemo>()));
