...
/// <c>Amazon*Config</c> shares; SWF needs none of its own — every operation already addresses a
/// single base endpoint and carries its domain or task list in the request body. Nothing else
/// about the SDK usage differs from production.
/// </summary>
public sealed class SwfClientFactory(AwsEndpoints endpoints)
{
    ...
    /// </summary>
    public IAmazonSimpleWorkflow Create()
    {
        ...
        {
            return new AmazonSimpleWorkflowClient(new AmazonSimpleWorkflowConfig
            {
        ...
        AmazonSimpleWorkflowConfig config = new AmazonSimpleWorkflowConfig
        {
            ...
            // A production app against real SWF wants the retries; this is the second and last
            // emulator-shaped line in the sample.
            MaxErrorRetry = 0,
        }.ForFloci(endpoints);

        return new AmazonSimpleWorkflowClient(endpoints.Credentials(), config);
    }
}
