...
public static class ServiceCollectionExtensions
{
    ...
    /// </summary>
    public static IServiceCollection AddAzureBlobDemo(this IServiceCollection services)
    {
        ArgumentNullException.ThrowIfNull(services);

        services.TryAddSingleton<BlobClientFactory>();

        // Registered by concrete type as well as by interface, because BlobPage injects BlobDemo
        ...
        services.TryAddSingleton<BlobDemo>();
        services.TryAddSingleton<BlobObjectStore>();
        ...
        // would register Blob twice if a host called this method twice. TryAddEnumerable
        // de-duplicates on the implementation type, which is both.
        services.TryAddEnumerable(
            ServiceDescriptor.Singleton<IServiceDemo, BlobDemo>(sp => sp.GetRequiredService<BlobDemo>()));
        services.TryAddEnumerable(
            ServiceDescriptor.Singleton<IObjectStoreCapability, BlobObjectStore>(
                sp => sp.GetRequiredService<BlobObjectStore>()));
