...
/// name it. Same helper as the AWS KMS sample's <c>KmsResponse</c> and the Secret Manager sample's
/// <c>SecretManagerResponse</c>; each sample keeps its own copy rather than sharing one, because a
/// sample has to stay clonable on its own.
/// </summary>
internal static class KmsResponse
{
    internal static T Require<T>(T? value, string operation, string field) where T : class
        => value ?? throw new InvalidOperationException($"{operation} answered, but without {field} — a field proto3 marks optional and real Cloud KMS always sends.");
}
