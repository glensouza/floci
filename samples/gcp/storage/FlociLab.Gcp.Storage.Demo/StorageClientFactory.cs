...
public sealed class StorageClientFactory(GcpEndpoints endpoints)
{
    ...
    /// </summary>
    public StorageClient Create() => new StorageClientBuilder
    {
        // The risk register's headline worry — that Google.Cloud.Storage.V1 would ignore a custom
        // BaseUri and there would be no way to reach the emulator short of hand-rolling an
        // HttpClient over the JSON API. It does not: verified end to end on 4.15.0 against
        ...
        // them. Without it the builder walks the ADC chain — GOOGLE_APPLICATION_CREDENTIALS, then
        // gcloud's well-known file, then the GCE metadata server — and fails on a developer
        // machine that has never seen gcloud, with an error about credentials rather than about
        // the endpoint. It is also the honest description of the emulator: it does not check.
        UnauthenticatedAccess = true,

        // Deliberately NOT setting EmulatorDetection. It does work here — see the class remarks
        // on StorageDemo — but it is driven by a process-wide environment variable, and a web app
        // that binds its endpoint from configuration should not be reaching for one.
    }.Build();
